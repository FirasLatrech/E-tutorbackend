  ...
  describe('GET /stripe/products', () => {
    it('should return products from StripeService', async () => {
      ...
      jest
        ...
        .mockResolvedValue(products as Stripe.Product[]);

      await request(app).get('/stripe/products').expect(200).expect(products);
    });
  });
  ...
  describe('GET /stripe/customers', () => {
    it('should return customers from StripeService', async () => {
      ...
      jest
        ...
        .mockResolvedValue(customers as Stripe.Customer[]);

      await request(app).get('/stripe/customers').expect(200).expect(customers);
    });
  });
